...
var resizePizzas = function(size) {
  ...
  // pizzaSize element is selected by Id name since web API calls are faster
  ...
  //to selects randomPizzaContainer element by class name and save to variable pizzaElements.
  // Moved dx and newwidth variables outside the for loop since it is not necessary to recompute them with each pass through loop.
  function changePizzaSizes(size) {
    ...
    //length of pizzaElements array is stored in variable len in initilization of for loop so that array length property is not accessed at each iteration.
...
//moved pizzDivs element outside the for loop  so that it doesnot repeat on DOM call on each pass.
...
function updatePositions() {
  ...
  //to select the moving pizzas element by class name
  var items = document.getElementsByClassName('mover');

  //moved the phase calculation into seperate for loop that appends each phase to an array, instead of declaring and setting the phase variable each time.
  phaseList = [];
  //created a local variable top outside for loop to prevent the DOM explictly touch in every iteration
  var top = (document.body.scrollTop || document.documentElement.scrollTop) / 1250;
  for (var j = 0; j < 5; j++){
    phaseList.push(Math.sin(top + (j % 5)));
  }

  //the pizza item styles are changed by accessing relevent element of phaselist
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //to change number of pizzas based on screen height
  var rows = Math.round(window.screen.height / s);
  var pizzaCount = rows * cols;
  //var elem is declared in the initilization of for loop to prevent it from being created every time the loop is executed
